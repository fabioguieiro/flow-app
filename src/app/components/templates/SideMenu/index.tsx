...
export const SideMenu = ({ handleCreateNewNode, title }: SideMenuProps) => {
  const [diamondBgActive,setDiamondBgActive ] = useState<boolean>(false)
  const [successBgActive,setSuccessBgActive ] = useState<boolean>(false)
  const [unsuccessBgActive,setUnsuccessBgActive ] = useState<boolean>(false)
    ...
    <aside className="bg-wesBeige h-full w-full border-l-4 border-black">
      <div className="flex h-full flex-col items-center justify-start">
        ...
        <div  className={`w-full h-2/6 flex items-center justify-between border-b-4 border-black `}>
          <div onClick={()=> setDiamondBgActive(!diamondBgActive)} className={`w-1/2 h-full flex justify-center items-center ${diamondBgActive ? 'bg-wesBeigeActive': 'bg-wesBeige'}`}>
          ...
          <div className="flex flex-col h-full items-center w-1/2 justify-around border-l-4 border-black">
            <div onClick={()=> setUnsuccessBgActive(!unsuccessBgActive)} className={`flex items-center justify-center border-b-4 border-black h-1/2 w-full ${unsuccessBgActive ? 'bg-wesBeigeActive': 'bg-wesBeige'}`}>
              <div className='bg-mrPink py-2 px-8 border-2 border-black rounded-lg tracking-widest text-black'>decision = false</div>
            </div>
            <div onClick={()=> setSuccessBgActive(!successBgActive)} className={`flex items-center justify-center h-1/2 w-full ${successBgActive ? 'bg-wesBeigeActive': 'bg-wesBeige'}`}>
